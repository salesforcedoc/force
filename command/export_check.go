...
var cmdExportCheck = &Command{
	Run:   runExportCheck,
	Usage: "export_check [options] [dir]",
	Short: "Checks Exports by Metadata Object to find which one is problematic",
	Long: `
Checks if exports are valid by Metadata Object
...
Examples:
  ...
  force export_check org/schema

  force export_check -x ApexClass -x CustomObject
`,
	MaxExpectedArgs: 1,
}
...
func init() {
	cmdExportCheck.Flag.BoolVar(&showWarnings, "w", false, "show warnings")
	cmdExportCheck.Flag.BoolVar(&showWarnings, "warnings", false, "show warnings")
	cmdExportCheck.Flag.Var(&excludeMetadataNames, "x", "exclude metadata type")
	cmdExportCheck.Flag.Var(&excludeMetadataNames, "exclude", "exclude metadata type")
	cmdExportCheck.Flag.Var(&includeMetadataNames, "i", "include only metadata type")
	cmdExportCheck.Flag.Var(&includeMetadataNames, "include", "include only metadata type")
	cmdExportCheck.Flag.BoolVar(&includeManagedPackages, "p", false, "include managed packages")
	cmdExportCheck.Flag.BoolVar(&includeManagedPackages, "package", false, "include managed packages")
}

func runExportCheck(cmd *Command, args []string) {
	...
	if (!isExcluded(customObject) && exportAll) || isIncluded(customObject) {
		...
		fmt.Printf("Listing CustomObjects...\n%s\n", stdObjects)
		sort.Strings(stdObjects)
